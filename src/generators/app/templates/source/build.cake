...
const string ModelProjectFolder = "Common\\<%= client %>.<%= package %>.Model";
const string ModelNamespace = "<%= client %>.<%= package %>.Model";
...
Task("BuildDeploymentProject")
  .Does(() => {
    BuildCSharpProject(
      File("Deploy/<%= client %>.<%= package %>.Deployment.csproj"), 
...
Task("BuildTestProjects")
  .Does(() => {
    var folderPrefix = "<%= client %>.<%= package %>.Tests.";
