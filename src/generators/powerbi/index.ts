...
class Main extends Generator {
  ...
  private updateBuildStageYmlFile = async (): Promise<void> => {
    ...
    const file : any = yaml.load(this.fs.read(buildStageYmlFilePath));
    const contentJson = JSON.parse(JSON.stringify(file));
    ...
    const updatedYaml = yaml.dump(contentJson, { lineWidth: -1, quotingType: "'" });
